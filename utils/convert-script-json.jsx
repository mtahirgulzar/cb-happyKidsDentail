...
import XMLData from "../utils/wp-data";
...
const Convert = () => {
  ...
  useEffect(() => {
    ...
    //   axios.post("https://admintoppinole.vigorant.xyz/api/services",{data:sin})
    ...
    const mapData = XMLData?.map((wpPost) => {
      ...
      for (let img of imgs) {
        img.src = `https://admindrkhayami.vigorant.xyz/uploads/${
      ...
      return {
        ...
        status: "published",
        // id: uuid(),
        title: turndownService.turndown(wpPost.title),
        description: turndownService.turndown(wpPost.excerpt),
        slug: wpPost.slug.slice(26, -1),
    ...
    mapData?.map((sin, index) => {
      // axios.post("https://admindrkhayami.vigorant.xyz/api/blog-datas", {
      //     data: {
      //         // id:sin.id,
      //         title: sin.title,
